...
#define STACK_ALLOC(n) sub STACK_REG, STACK_REG, n
#define STACK_FREE(n) add STACK_REG, STACK_REG, n

#define PUSH(reg) STACK_ALLOC(1); store [STACK_REG], reg;
#define POP(reg) load reg, [STACK_REG]; STACK_FREE(1);
...
#define CALL(func) CALL_IMPL(func,CONCAT(Call_Return_,__COUNTER__))
